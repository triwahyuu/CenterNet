    ...
    for img_id in images:
        ...
        for ann in anns:
            if ann['iscrowd'] > 0:
                continue
            bbox = ann['bbox']
            center = ((bbox[0] + bbox[2] / 2) // 4, (bbox[1] +
                                                     bbox[3] / 2) // 4, ann['category_id'], bbox)
            for c in centers:
...
def count_iou(split):
    ...
    for img_id in images:
        ...
        for ann in anns:
            if ann['iscrowd'] > 0:
                continue
            bbox = _coco_box_to_bbox(
                ann['bbox']).tolist() + [ann['category_id']]
            for b in bboxes:
